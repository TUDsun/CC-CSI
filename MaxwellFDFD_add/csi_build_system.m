%% csi_build_system
% corrected version of build_system.m, corrections are as follows
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   obj_array, src_array, mat_array, eps_node, mu_node, isiso] = mybuild_system(varargin)
...
%     mu_node_temp = ones(size(arg));
%     mu_node_cell = {mu_node_temp, mu_node_temp, mu_node_temp};
% elseif istypesizeof(arg, 'complex', [0 0 0 0])
%     chi = arg;
%     chiflag = true;
% else
...
pml = arg;
    ...
    function material = create_material(varargin)
        ...
        matname = varargin{1};
        if ~contains(matname, '/')  % data table is not specified
